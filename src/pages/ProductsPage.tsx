...
import { getProducts, addProduct, updateProduct, deleteProduct, deleteAllProducts, setProductDiscount } from '@/services/products';
...
export default function ProductsPage() {
    ...
    try {
      ...
      if (editProduct) {
        await updateProduct(editProduct.id, {
          barcode: barcode.trim(), name: name.trim(), unit, cost: costN, price: priceN,
  ...
  const handleDeleteAll = async () => {
    if (!shop?.id) return;
    try {
      await deleteAllProducts(shop.id);
      toast.success('සියලුම භාණ්ඩ ඉවත් කළා');
      setDeleteAllConfirmOpen(false);
      load();
    } catch { toast.error('සියලුම භාණ්ඩ ඉවත් කිරීම අසාර්ථකයි'); }
  };
    ...
    <AppLayout>
      <div className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-balance">{t('products.title')}</h2>
          </div>
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            {isAdmin && (
              <>
                ...
                <Button variant="destructive" size="sm" onClick={() => setDeleteAllConfirmOpen(true)} className="gap-1.5">
                  <Trash2 className="w-3.5 h-3.5" />
                  <span className="sr-only md:not-sr-only">සියල්ල මකන්න</span>
                </Button>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="whitespace-nowrap">{t('products.unit')}</TableHead>
                        {isAdmin && <TableHead className="whitespace-nowrap">{t('products.cost')}</TableHead>}
                        ...
                        <TableHead className="whitespace-nowrap">{t('products.supplier')}</TableHead>
                              ...
                              {isAdmin && (
                                <TableCell className="whitespace-nowrap text-right">
                                  <div className="flex items-center justify-end gap-1">
                                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setPrintProduct(p)}>
          ...
          <div className="space-y-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5 col-span-2">
                <Label className="text-sm font-normal">{t('products.barcode')}</Label>
                <div className="flex gap-2">
                  ...
                  {!form.barcode && (
                    <Button 
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        const generated = Math.floor(Math.random() * 1000000000000).toString().padStart(12, '0');
                        setForm(f => ({ ...f, barcode: generated }));
                      }}
                      className="shrink-0"
                    >{t('products.generate_barcode')}</Button>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-1.5 col-span-2">
                ...
                <Select value={form.branch_id} onValueChange={v => setForm(f => ({ ...f, branch_id: v }))}>
                  <SelectTrigger><SelectValue placeholder={t('products.branch')} /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">— නැත —</SelectItem>
      ...
      <AlertDialog open={deleteAllConfirmOpen} onOpenChange={setDeleteAllConfirmOpen}>
        <AlertDialogContent className="max-w-[calc(100%-2rem)] md:max-w-md">
          <AlertDialogHeader>
            <AlertDialogTitle>සියලුම භාණ්ඩ ඉවත් කරන්න ද?</AlertDialogTitle>
            <AlertDialogDescription>
              මෙමගින් ඔබගේ වෙළඳසැලේ ලියාපදිංචි කර ඇති සියලුම භාණ්ඩ සම්පූර්ණයෙන්ම ඉවත් කරනු ඇත. මෙම ක්‍රියාව නැවත හැරවිය නොහැක.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
